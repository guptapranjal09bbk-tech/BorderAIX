...
def security_gate():
    ...
    if submitted:
        ...
        if ok_id and ok_pw:
            ...
            audit_event("SUCCESSFUL_LOGIN", {"user_id": gov_id, "failed_attempts_before_success": record.get("failed_attempts", 0)})
            append_block("SUCCESSFUL_LOGIN", {"user_id": gov_id, "failed_attempts": record.get("failed_attempts", 0)})
            ...
            audit = audit_event("FAILED_LOGIN", {"attempt": failures, "risk": risk, "user_id": gov_id[:64]})
            append_block("FAILED_LOGIN", {"attempt": failures, "risk": risk, "incident_id": audit["id"]})

            if failures == 1:
                st.warning("⚠️ Warning: Invalid credentials. First failed login attempt recorded.")
            elif failures == 2:
                sms = (
                ...
                st.error("🚨 SUSPICIOUS LOGIN ACTIVITY — Attempt 2 detected.")
                alarm()
    ...
    if record.get("failed_attempts", 0):
        st.caption(f"Recorded failed attempts: {record['failed_attempts']} | Current risk: {min(100, 40 + record['failed_attempts'] * 20)}%")
    st.stop()
